...
Figure~\ref{fig:base-arch} shows the baseline GPT-2 decoder-only architecture.
Figure~\ref{fig:qat-arch} gives an overview of the full quantized training pipeline built on top of it.
...
head. The project preserves the original \texttt{llm.c} mixed-precision convention.
Forward and backward tensors use \texttt{floatX}, which is selected at compile time as
