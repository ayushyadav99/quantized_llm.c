...
// defines: quantize_weights, dequantize_weights (floatLOAD <-> int8 helpers)
...
// ParameterTensorsLoad mirrors ParameterTensors but uses floatLOAD* pointers.
// Weights are loaded from disk into this struct, allowing a separate quantization
// step before copying into the main model's ParameterTensors (floatX).
typedef struct {
    floatLOAD* wte; // (V, C)
    floatLOAD* wpe; // (maxT, C)
    floatLOAD* ln1w; // (L, C)
    floatLOAD* ln1b; // (L, C)
    floatLOAD* qkvw; // (L, 3*C, C)
    floatLOAD* qkvb; // (L, 3*C)
    floatLOAD* attprojw; // (L, C, C)
    floatLOAD* attprojb; // (L, C)
    floatLOAD* ln2w; // (L, C)
    floatLOAD* ln2b; // (L, C)
    floatLOAD* fcw; // (L, 4*C, C)
    floatLOAD* fcb; // (L, 4*C)
    floatLOAD* fcprojw; // (L, C, 4*C)
    floatLOAD* fcprojb; // (L, C)
    floatLOAD* lnfw; // (C)
    floatLOAD* lnfb; // (C)
} ParameterTensorsLoad;
static_assert(sizeof(ParameterTensorsLoad) == NUM_PARAMETER_TENSORS * sizeof(void*), "Inconsistent sizes!");
...
// GPT2Load holds loaded weights in floatLOAD precision.
typedef struct {
    GPT2Config config;
    ParameterTensorsLoad params;
    size_t param_elements[NUM_PARAMETER_TENSORS];
    size_t param_sizeof[NUM_PARAMETER_TENSORS]; // sizeof(floatLOAD) for each tensor
    void* params_memory;          // single GPU allocation backing all params
    size_t num_parameters;
    size_t num_parameters_bytes;
} GPT2Load;

void gpt2_load_init(GPT2Load *model) {
    model->params_memory = NULL;
}
...
// Allocate a single contiguous GPU buffer for ParameterTensorsLoad and point each
// tensor pointer into it. All elements are sizeof(floatLOAD) bytes each.
void* malloc_and_point_parameters_load(ParameterTensorsLoad* params, size_t* param_elements, size_t num_params) {
    size_t num_parameters_bytes = 0;
    for (int i = 0; i < num_params; i++) {
        num_parameters_bytes += param_elements[i] * sizeof(floatLOAD);
    }
    void* params_memory;
    cudaCheck(cudaMalloc((void**)&params_memory, num_parameters_bytes));
    floatLOAD** ptrs[] = {
    ...
    char* mem_iter = (char*)params_memory;
    for (int i = 0; i < (int)num_params; i++) {
        *(ptrs[i]) = (floatLOAD*)mem_iter;
        mem_iter += param_elements[i] * sizeof(floatLOAD);
    }
    return params_memory;
}

// Load model weights from a checkpoint file into a GPT2Load struct.
// The weights are stored on the GPU in floatLOAD precision, ready for
// a subsequent quantization step into floatX.
void gpt2_load_from_checkpoint(GPT2Load *load, const char* checkpoint_path, bool weight_init=true) {
    
    if (weight_init && LOAD_PRECISION_MODE == PRECISION_FP16) {
    ...
    // Verify the on-disk precision matches floatLOAD so we can load without conversion.
    if (weight_init && LOAD_PRECISION_MODE == PRECISION_BF16 && version != 5) {
        fprintf(stderr, "floatLOAD is BF16 but checkpoint at %s is not BF16 (version %d).\n", checkpoint_path, version);
        fprintf(stderr, "---> HINT: are you sure you're loading a _bf16.bin file?\n");
        exit(EXIT_FAILURE);
    }
    if (weight_init && LOAD_PRECISION_MODE == PRECISION_FP32 && version != 3) {
        fprintf(stderr, "floatLOAD is FP32 but checkpoint at %s is not FP32 (version %d).\n", checkpoint_path, version);
        fprintf(stderr, "---> HINT: compile with LOAD_PRECISION=FP32 (-DENABLE_LOAD_FP32) or load a .bin without _bf16 in the name.\n");
        exit(EXIT_FAILURE);
    }

    // Read hyperparameters.
    load->config.max_seq_len = model_header[2];
    load->config.vocab_size  = model_header[3];
    load->config.num_layers  = model_header[4];
    load->config.num_heads   = model_header[5];
    load->config.channels    = model_header[6];
    load->config.padded_vocab_size = model_header[7];

    // Fill in element counts (reuse existing helper; param_sizeof values are unused here).
    fill_in_parameter_sizes(load->param_elements, load->param_sizeof, load->config);
    load->num_parameters = 0;
    load->num_parameters_bytes = 0;
    for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
        load->num_parameters += load->param_elements[i];
        load->num_parameters_bytes += load->param_elements[i] * sizeof(floatLOAD);
    }

    if (weight_init) {
        // Allocate GPU memory and point tensors into it.
        assert(load->params_memory == nullptr);
        load->params_memory = malloc_and_point_parameters_load(&load->params, load->param_elements, NUM_PARAMETER_TENSORS);

        // Stream weights from disk directly onto the GPU.
        file_to_device(load->params_memory, model_file, load->num_parameters_bytes, IO_BUF_SIZE, main_stream);
        cudaCheck(cudaDeviceSynchronize());
        printf0("Loaded %zu parameters (%.1f MiB) from %s into floatLOAD GPU buffer\n",
                load->num_parameters, load->num_parameters_bytes / (1024.0 * 1024.0), checkpoint_path);
    }
    fcloseCheck(model_file);
}

// Free GPU memory held by a GPT2Load struct.
void gpt2_load_free(GPT2Load *load) {
    if (load->params_memory != nullptr) {
        cudaCheck(cudaFree(load->params_memory));
        load->params_memory = nullptr;
    }
}
...
void gpt2_quantize_loaded_model(GPT2 *model, GPT2Load *load_model) {
    ...
    dsts[1]  = model->params.wpe;       srcs[1]  = load_model->params.wpe;
    dsts[2]  = model->params.ln1w;      srcs[2]  = load_model->params.ln1w;
    dsts[3]  = model->params.ln1b;      srcs[3]  = load_model->params.ln1b;
    dsts[4]  = model->params.qkvw;      srcs[4]  = load_model->params.qkvw;
    dsts[5]  = model->params.qkvb;      srcs[5]  = load_model->params.qkvb;
    dsts[6]  = model->params.attprojw;  srcs[6]  = load_model->params.attprojw;
    dsts[7]  = model->params.attprojb;  srcs[7]  = load_model->params.attprojb;
    dsts[8]  = model->params.ln2w;      srcs[8]  = load_model->params.ln2w;
    dsts[9]  = model->params.ln2b;      srcs[9]  = load_model->params.ln2b;
    dsts[10] = model->params.fcw;       srcs[10] = load_model->params.fcw;
    dsts[11] = model->params.fcb;       srcs[11] = load_model->params.fcb;
    dsts[12] = model->params.fcprojw;   srcs[12] = load_model->params.fcprojw;
    dsts[13] = model->params.fcprojb;   srcs[13] = load_model->params.fcprojb;
...
void gpt2_build_from_checkpoint(GPT2 *model, const char* checkpoint_path, bool weight_init=true, bool quantize=true) {
    ...
    if (quantize) {
        GPT2Load load_model;
        gpt2_load_init(&load_model);
        gpt2_load_from_checkpoint(&load_model, checkpoint_path, weight_init);

        // Allocate model weight memory first, then cast floatLOAD -> floatX into it.
        model->config = load_model.config;
        gpt2_allocate_weights(model);
        gpt2_quantize_loaded_model(model, &load_model);
        gpt2_load_free(&load_model);
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    bool quantize_from_load = false;

    #ifdef LOAD_PRECISION_MODE
        // quantization load path
        quantize_from_load = true;
        if (resuming == 1) {
            printf0("Resuming from checkpoint, ignoring LOAD_PRECISION_MODE and loading in full precision for resumption.\n");
            quantize_from_load = false;
        }
    #endif
    ...
    if (resuming == 1) {
        ...
        gpt2_build_from_checkpoint(&model, filename_buffer, weight_init, quantize_from_load);
    } else if (ends_with_bin(load_filename)) {
        // otherwise, if this is a .bin file, we assume it's a model, let's init from it
        gpt2_build_from_checkpoint(&model, load_filename, quantize_from_load=quantize_from_load);
    } else {
